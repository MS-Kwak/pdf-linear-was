...
function getOrRegisterFile(filePath: string): string {
  const existing = db
    .prepare('SELECT id FROM pdf_files WHERE file_path = ?')
    .get(filePath) as PdfFileRow | null;
  if (existing) return existing.id;

  const id = uuidv4();
  db.prepare(
    'INSERT INTO pdf_files (id, file_name, company, file_path) VALUES (?, ?, ?, ?)',
...
async function linearize(filePath: string): Promise<void> {
  ...
  try {
    ...
    console.log(
      `[watcher] Linearize 완료: ${path.basename(filePath)}`,
    );
  } catch {
...
async function handleFile(filePath: string) {
  ...
  if (isNew) {
    ...
    await linearize(filePath);
    linearizeMs = Date.now() - linStart;
  }
    ...
    try {
      const res = await fetch(MNWISE_API_URL, {
        ...
        headers: {
          ...
          Authorization: `Bearer ${MNWISE_API_KEY}`,
        },
        body: JSON.stringify({ fileName, viewerUrl: url }),
      });
      if (!res.ok)
        console.error(`[watcher] 엠엔와이즈 오류: ${res.status}`);
